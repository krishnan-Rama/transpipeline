...
upimapi --blast -i "${blastout}/${assembly}_\${i}_blp.tsv" -t ${SLURM_CPUS_PER_TASK} -o ${upimapi}/\${i}/ -ot "${upimapi}/\${i}/${assembly}_\${i}_upimapi.tsv" --columns "IMGT_GENE-DB&Gene ontology (biological process)"

# Add the output file to your array
result_array+=("${upimapi}/\${i}/${assembly}_\${i}_upimapi.tsv")

done

# Create a CSV file from the array
printf "%s\n" "\${result_array[@]}" > output_results.csv
